...
/**
 * Generic class which can be used to represent the Graph data type. Includes shortest path algorithm.
 *
 * @author Drew Paterno
 *
 */
public class Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      this.dataSequence = new LinkedList<>();
      for (T x : source.dataSequence)
        this.dataSequence.add(x);
      this.end = source.end;
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    // Checks for invalid start input.
    if (vertices.get(start) == null)
      throw new NoSuchElementException("Start key is not in the graph.");

    // Initializes the priority queue and the list of visited nodes.
    PriorityQueue<Path> pq = new PriorityQueue<Path>();
    LinkedList<T> visited = new LinkedList<T>();
    pq.add(new Path(vertices.get(start))); // Adds the first Path (containing the start vertex) to
                                           // the queue.

    // The loop will terminate when the next Path to be removed from the PQ is a path from the start
    // node to the end node.
    while (!pq.peek().end.data.equals(end)) {
      Path current = pq.poll(); // Dequeue
      visited.add(current.end.data); // Add dequeued node to the list of visited nodes.
      List<Edge> edges = current.end.edgesLeaving; // Get list of edges from the current end of the
                                                   // path.
      for (Edge x : edges) {
        // Adds new Paths to the queues containing Paths with the end set as any unvisited successor
        // of the current path's end node.
        if (!visited.contains(x.target.data)) {
      ...
      // Throws exception if the queue is empty (no path was found).
      if (pq.peek() == null) {
        throw new NoSuchElementException(
            "The end vertex provided is unreachable from the start vertex provided.");
      }

    }
    return pq.peek(); // Returns the shortest path from start to end, as computed by Dijkstra's
                      // algorithm.
  }
